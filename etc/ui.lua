...
function library:CreateWatermark(name, position)
    ...
    runservice.Heartbeat:Connect(function()
        ...
        if not name:find("{fps}") then
            watermark.label.Text = " " .. name:gsub("{game}", gamename):gsub("{fps}", "0 FPS"):gsub("{ping}", "0ms") .. " "
        end
        ...
        if name:find("{fps}") then
            ...
            if currentTime - startTime >= 1 then 
                ...
                if fps ~= oldfps then
                    watermark.label.Text = " " .. name:gsub("{game}", gamename):gsub("{fps}", fps .. " FPS"):gsub("{ping}", game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()) .. " "
